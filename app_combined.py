...
green"))
    ...
    if not df_operacional_filtrado.empty:
        ...
        st.subheader(f"Total de Chamados: {total_chamados_geral}")
        ...
        for i, analista in enumerate(analistas_especificos):
            with cols_analistas[i]:
                ...
                st.metric("Atendimentos dia", atendimentos_dia)
                st.metric("TMA", f"{tma:.0f} min")
                st.metric("CSAT", f"{csat:.0f}%")
                st.metric("% Resposta Pesquisa", f"{percentual_resposta_pesquisa:.0f}%")
                st.metric("SLA 1º Atendimento", f"{sla_primeiro_atendimento:.0f}%")
                st.metric("SLA Resolução", f"{sla_resolucao:.0f}%")
...
# --- Conteúdo das Páginas (continuação) ---
